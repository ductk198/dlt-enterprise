...
(function ( $, window, document, undefined ) {
    ...
    var pluginName = 'dropzone',
        defaults = {
            ...
            text:                   '<div class="drop-file-tittle" row-fluid"><div class="drop-icon"><i class="material-icons">cloud_upload</i></div><div class="span12 text-center"><h2>Kéo thả file vào đây</h2><br><h3>Hoặc sử dụng nút "Tải lên"</h3></div></div>',    //text inside the div
    ...
    function upload(that, files){
        if(that.options.preview){
            if(!checkFileType(that, files[0])){
                if(typeof that.options.error == "function"){
                    that.options.error($(that.element), "fileNotAllowed", "Hệ thống không hỗ trợ định dạng file này! Vui lòng liên hệ với người quản trị để được trợ giúp");
                }else
                    alert("Hệ thống không hỗ trợ định dạng file này! Vui lòng liên hệ với người quản trị để được trợ giúp");
                return false;
            }
            if(!checkFileSize(that, files[0])) {
                if(typeof that.options.error == "function"){
                    that.options.error($(that.element), "fileToBig", 'Dung lượng file quá lớn ('+humanFileSize(files[0].size)+')! Dung lượng tối đa là ('+that.options.maxFileSize+')');
                }else
                    alert('Dung lượng file quá lớn ('+humanFileSize(files[0].size)+')! Dung lượng tối đa là ('+that.options.maxFileSize+')');
                return false;
            }
        ...
        if(files){
            ...
            if(that.options.uploadMode == 'all'){
            	...
            	$("#progess-count-file").html("File tải lên <b>("+"<span id='progess-count-file-success'></span>"+totalfile+" file)</b>");
                for (i = 0; i < files.length; i++) {
                   // timerStartDate[uploadIndex] = $.now();
                    ...
                    formData.append('id_user', that.options.userid);
                    formData.append('id_document', that.options.id_document);
                            ...
                            xhr.onreadystatechange = function()
                    	  	{
                    	    	if (xhr.readyState == 4 && xhr.status == 200)
                    	    	{
                    	    		...
                    	    		EfyLib.alertMessage('danger','Tên tài liệu đã tồn tại, vui lòng đẩy tài liệu khác!',6000);
                    	    		EfyLib.successLoadImage();
                    	    	}else if(xhr.status == 409){
                    	    		$(".progess-main").hide();
                   	    		 	EfyLib.alertMessage('danger','Bạn không có quyền tải file ở thư mục này!',6000);
	                 	    		EfyLib.successLoadImage();
	                 	    		
	                 	    	}
                    	  	};
                            xhr.open('post', that.options.url);
                            xhr.setRequestHeader('X-CSRF-TOKEN', that.options.token);
        ...
        $(xhr.upload).bind({
            progress: function(e){
                if(e.originalEvent.lengthComputable){
                    ...
                    percent = percent - randomBetween(2,10);
                    if(percent > 0){
    ...
    function addProgressBar(that, i, index){
    	var order = i+1;
    	$(".progess-main").show();
        $('.showprogess').append('<div name-file="'+that.options.files[i].name+'" class="progress progress-bar-success progress-'+index+'"></div>').css({'margin': that.options.margin});
        $(".progress-"+index).append('<div class="progress-bar"></div>').hide();
        $(".progress-" + index).wrap('<div class="extra-progress-wrapper"></div>');
        $(".progress-" + index).parent().append('<span id-progress="progress-'+index+'" class="progress-filename" title="'+that.options.files[i].name+'"><b> File '+order+ '</b>: ' +that.options.files[i].name.trunc(20)+'</span>').css("width", that.options.progressBarWidth);
    ...
    function addFileToBigField(that, i, index){
    	$(".progess-main").show();
        $('.showprogess').append('<div class="progress  error-progress-'+index+'"></div>').css('margin', that.options.margin);
        var file = that.options.files[i];
        var fileName = file.name.trunc(25);
        $(".error-progress-"+index).css({
            width: that.options.progressBarWidth,
            margin: '20px 0 0 0'
        }).append('<div class="progress-bar progress-bar-danger" style="width:100%">Dung lượng file vượt quá cho phép ('+humanFileSize(file.size)+')</div>');
    ...
    function addWrongFileField(that, i, index){
        ...
        $(".error-progress-"+index).css({
            width: that.options.progressBarWidth,
            margin: '20px 0 0 0'
        }).append('<div class="progress-bar progress-bar-danger" style="width:100%">Định dạng file ('+extension+') không được hỗ trợ!</div>');
        $(".error-progress-" + index).wrap('<div class="extra-progress-wrapper"></div>').css("width", that.options.progressBarWidth);
        $(".error-progress-" + index).parent().append('<span title="'+that.options.files[i].name+'">'+fileName+'</span>');
    }
